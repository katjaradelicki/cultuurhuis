...
<body>
	<h1>
		Het Cultuurhuis: bevestiging reservatie <img alt="bevestig.png"
		...
		<dl>
			...
			<dd>
				<input type="text" name="gebruikersnaam"
					<c:if test="${not empty klant}">  disabled="disabled" </c:if> />
			</dd>
			<dt>Paswoord:</dt>
			<dd>
	...
	<form action="${nieuweKlantUrl}" method="get">
		<%-- alternatief: link opmaken als knop met css  -->Nadeel:knop kan wijzigen in uitzicht (browser,versie,...) maar de css-knop niet (tenzij je verschillende css' voorziet) --%>
	...
	<form action="${toevoegenUrl}" method="post">
		<input type="submit" value="Bevestigen"
			<c:if test="${empty klant}">  disabled="disabled" </c:if>>
	</form>
</body>
</html>